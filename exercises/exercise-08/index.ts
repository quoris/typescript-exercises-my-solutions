...
//     status: string
// }
// type AdminsApiResponseSuccess = AdminsApiResponse & { data: Admin[] }
// type AdminsApiResponseError = AdminsApiResponse & { error: string }
// 3rd way
// type AdminsApiResponse = {
//     status: string;
//     data?: Admin[]
...
function requestCurrentServerTime(callback: (response: AdminsApiResponse) => void) {
    callback({
        status: 'success',
        data: Date.now()
    });
...
// response can be AdminsApiResponse or UsersApiResponse type
function requestCoffeeMachineQueueLength(callback: (response: AdminsApiResponse<User>) => void) {
    callback({
        status: 'error',
